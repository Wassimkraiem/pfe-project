...
The work was split into one requirements sprint (Sprint 0) followed by five implementation sprints.
...
Sprint 2 & Video Indexing and Full-Stack Search & Delivered OpenSearch ingestion, BM25 keyword search, and facet filters (Part A), then added semantic embeddings, kNN vector retrieval, parallel hybrid search, and position-aware rank fusion (Part B). \\
Sprint 3 & Agentic Helper: RAG, Classification, Search Orchestration and Voice Transcription & Built the RAG pipeline with Qdrant, two-level routing/classification, and conversational memory (Phase A), then upgraded to the v2 deterministic orchestrator, Agentic RAG with CRAG-pattern re-retrieval, FAQ vector administration, and the Whisper voice transcription endpoint (Phase B). \\
Sprint 4 & Next.js Portals & Delivered the Video Search Portal (Next.js 15) and Authentic Rights Portal (Next.js 16) wired to all backend services, with the portal-side mic button, the shared ChatbotWidget, and the admin workspace. \\
Sprint 5 & Personalized Recommendation Engine & Implemented user-interest profiling from events, candidate generation + blending, trending integration, and anti-repetition ranking in recommendations. \\
\bottomrule
\end{tabularx}
\end{table}
...
The Gantt chart in Figure~\ref{fig:gantt} shows the planned distribution of the six sprints over
...
\draw[bar]  (14,-5.8)  rectangle (16,-6.4);  \node[lbl] at (-0.2,-6.1) {Sprint 5};
\end{tikzpicture}%
}
...
\begin{itemize}
  ...
  \item \textbf{Sprint 2}: Delivered OpenSearch ingestion, BM25 keyword search, facet-based
    ...
    position-aware rank fusion (Part B).
  \item \textbf{Sprint 3}: Built the Agentic Helper RAG foundation with Qdrant retrieval,
    ...
    endpoint (Phase B).
  \item \textbf{Sprint 4}: Delivered the Video Search Portal and Authentic Rights Portal wired
    to all backend services, with the portal-side mic integration and the admin workspace.
  \item \textbf{Sprint 5}: Implemented recommendation event tracking, user-interest profiling,
