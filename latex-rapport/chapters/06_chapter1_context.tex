...
\subsection{Agile Approach: Scrum-Inspired Sprints}

The project used an \textbf{Agile} workflow organized into two-week sprints. Each sprint started
with a planning session (user stories, acceptance criteria) and ended with a review and
retrospective. The product backlog was maintained and prioritized together with the industrial
supervisor.
...
The work was split into one requirements sprint (Sprint 0) followed by six implementation sprints.
...
Sprint 1 & Onboarding, Auth, and Billing Core & Implemented authentication and onboarding workflows, integrated Stripe subscription payment, and added AR-API background processing with Celery tasks. \\
Sprint 2 & Video Indexing and Keyword Search & Delivered OpenSearch video ingestion, mapping and aliases, BM25 keyword search, and facet filters for portal browsing. \\
Sprint 3 & Semantic Search and Hybrid Ranking & Added embedding generation, kNN vector retrieval, parallel lexical + semantic search, and rank fusion with position-aware scoring. \\
Sprint 4 & RAG Chatbot Foundation & Built the Agentic Helper RAG pipeline with Qdrant retrieval, routing/classification logic, and conversational memory support. \\
Sprint 5 & Agentic Search, Voice, and FAQ Admin & Delivered ReAct search orchestration, Whisper transcription, chatbot integration in the portals, and FAQ management endpoints/interfaces. \\
Sprint 6 & Personalized Recommendation Engine & Implemented user-interest profiling from events, candidate generation + blending, trending integration, and anti-repetition ranking in recommendations. \\
\bottomrule
\end{tabularx}
\end{table}
...
The Gantt chart in Figure~\ref{fig:gantt} shows the planned distribution of the seven sprints over
...
%% Horizontal axis: 14 weeks
\foreach \x in {0,...,14} {
  \draw[axis, dashed] (\x,-0.3) -- (\x,-7.7);
}
\foreach \x/\w in {0/W1, 2/W3, 4/W5, 6/W7, 8/W9, 10/W11, 12/W13, 14/W15} {
  \node[wk] at (\x,0.1) {\w};
}

%% Sprint bars (start, end, y, style, label)
\draw[bar0] (0,-0.8) rectangle (2,-1.4);   \node[lbl] at (-0.2,-1.1) {Sprint 0};
\draw[bar]  (2,-1.8) rectangle (4,-2.4);   \node[lbl] at (-0.2,-2.1) {Sprint 1};
\draw[bar]  (4,-2.8) rectangle (6,-3.4);   \node[lbl] at (-0.2,-3.1) {Sprint 2};
\draw[bar]  (6,-3.8) rectangle (8,-4.4);   \node[lbl] at (-0.2,-4.1) {Sprint 3};
\draw[bar]  (8,-4.8) rectangle (10,-5.4);  \node[lbl] at (-0.2,-5.1) {Sprint 4};
\draw[bar]  (10,-5.8) rectangle (12,-6.4); \node[lbl] at (-0.2,-6.1) {Sprint 5};
\draw[bar]  (12,-6.8) rectangle (14,-7.4); \node[lbl] at (-0.2,-7.1) {Sprint 6};
\end{tikzpicture}
...
\begin{itemize}
  ...
  \item \textbf{Sprint 1}: Implemented authentication and onboarding flows, integrated Stripe
    subscriptions, and added AR-API background processing with Celery tasks.
  \item \textbf{Sprint 2}: Delivered OpenSearch ingestion and index setup, BM25 keyword search,
    and facet-based filtering for browsing.
  \item \textbf{Sprint 3}: Added embedding generation, kNN vector retrieval, parallel lexical and
    semantic search, and hybrid ranking.
  \item \textbf{Sprint 4}: Built the Agentic Helper RAG foundation with Qdrant retrieval,
    routing/classification, and conversation memory.
  \item \textbf{Sprint 5}: Delivered ReAct agentic search orchestration, Whisper transcription,
    chatbot portal integration, and FAQ administration interfaces.
  \item \textbf{Sprint 6}: Implemented recommendation event tracking, user-interest profiling,
