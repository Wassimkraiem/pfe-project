...
produce the final ranking before pagination. The sprint also delivers the \textbf{Video Search
Portal} front-end (Next.js 15) that exposes the full search and browsing experience to users,
as well as the playlist management views of the Authentic Rights Portal.
...
diagram (Figure~\ref{fig:uc_global} in Chapter~2). Figure~\ref{fig:uc_sprint2} details the
sprint-level use cases.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{\detokenize{diagrams/sprint 2 use.drawio.png}}
\caption{Use-case diagram — video indexing and full-stack search}
\label{fig:uc_sprint2}
\end{figure}

\subsection{Class Diagram}

Figure~\ref{fig:cd_sprint2} shows the data model and service structure for the Videos Search
API. The core entities are the OpenSearch video document schema, the search request/response
models, and the ranking pipeline components.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{\detokenize{diagrams/sprint 2 class.drawio.png}}
\caption{Class diagram — Videos Search API data model and ranking pipeline}
\label{fig:cd_sprint2}
\end{figure}
...
The Video Search Portal (\texttt{videosearchv0/}) is the public-facing interface for video
discovery and browsing:

\begin{itemize}
  \item \textbf{Next.js 15} with the App Router and React Server Components.
  \item \textbf{Radix UI} primitives and \textbf{Tailwind CSS} for a dark, content-first UI.
  ...
  \item BFF routes (\texttt{app/api/chat/route.ts}, \texttt{app/api/transcribe/route.ts}) that
    forward requests to the Agentic Helper and attach the required API key and user headers.
\end{itemize}
...
\subsection{Delivered Interfaces — Portal and Playlists}
...
The Video Search Portal exposes this engine through a polished public landing page, authenticated
dashboard, category browsing, and video detail pages, while the Authentic Rights Portal delivers
playlist management. The next sprint moves up one abstraction layer and implements the Agentic
Helper's RAG and intent classifiers on top of this search foundation.
