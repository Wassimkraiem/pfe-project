...
voice transcription endpoint. The portal-side integration — voice input, the shared chatbot
widget, and AI-powered admin tools — is also delivered as part of this sprint.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{\detokenize{diagrams/sprint 3 use.drawio.png}}
\caption{Use-case diagram — Agentic Helper: RAG, classification and voice transcription}
\label{fig:uc_chat}
\end{figure}
...
database. Figure~\ref{fig:cd_sprint3} shows the full class structure.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{\detokenize{diagrams/sprint 3 class.drawio.png}}
\caption{Class diagram — Agentic Helper service architecture}
\label{fig:cd_sprint3}
\end{figure}
...
built in Phase B (see Figure~\ref{fig:seq_transcribe}). The portal-side flow is:
...
\subsection{Shared ChatbotWidget}
...
\subsection{AI Admin Tools}

The Authentic Rights Portal admin workspace includes three AI-backed management surfaces that
consume the Agentic Helper directly through BFF routes:

\begin{itemize}
  \item \textbf{Global conversation supervision}: an admin-only AR-API endpoint
    (\texttt{GET /conversations/admin/all}) aggregates conversation histories across users so
    the support team can inspect titles, participants, message counts, and latest activity.
  ...
  \item \textbf{FAQ management on vector storage}: the FAQ manager is connected to the RAG
    vector store rather than a static list. Through BFF routes, the portal consumes
...
transcription endpoint (Phase B). The portal-side integration completes the sprint: both portals
gained a voice input flow via the Whisper BFF route and a shared ChatbotWidget, while the
Authentic Rights Portal admin workspace gained AI-backed conversation supervision, a chatbot
tester, and a vector-backed FAQ manager. The next sprint implements the personalized
recommendation engine on top of this complete foundation.
