...
function SignupReviewStepContent() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(
        `/signup/review/thank-you?email=${encodeURIComponent(email)}`
      );
    } catch (err) {
      setSubmitError(
        ...
        <Box sx={{ mx: "auto", mt: { xs: 4, sm: 6 }, maxWidth: "42rem" }}>
          ...
          >
            <Button
              ...
              href={
                email
                  ? `/signup/pages?email=${encodeURIComponent(email)}`
                  : "/signup/pages"
              }
              fullWidth
              variant="outlined"
